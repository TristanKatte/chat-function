/**
 * A node that renders its children as a clickable {@link https://talkjs.com/docs/Features/Customizations/Action_Buttons_Links/ | action button} which triggers the specified custom action.
 *
 * @public
 */
...
/**
 * A node that renders its children as a clickable {@link https://talkjs.com/docs/Features/Customizations/Action_Buttons_Links/ | action link} which triggers the specified custom action.
 *
 * @public
 */
...
/**
 * A node that renders its children styled as a quote.
 *
 * @public
 */
 ...
 * These blocks should be rendered in order, top-to-bottom.
 *
...
export declare interface ConversationRef {
     ...
     *
     * @returns A promise that resolves with a reference to the newly created message. The promise will reject if you are not a participant with write access in this conversation.
     */
    send(params: string | SendTextMessageParams): Promise<MessageRef>;
    /**
...
export declare interface ConversationSnapshot {
     ...
     *
     * @remarks
     * If no messages have been sent, this will be the date that the conversation was created instead.
     * To show a chronological list of recent conversations, sort by this property.
     */
...
/**
 * A node that is used for {@link https://talkjs.com/docs/Features/Message_Features/Emoji_Reactions/#custom-emojis | custom emoji}.
 *
 * @public
 */
...
/**
 * A node that is used for emoji, both unicode emoji like 😛 and plain-text emoticons like :P
 *
 * @public
 */
export declare interface EmojiNode {
    type: "emoji";
 ...
 * @public
 */
export declare interface FileBlock {
    type: "file";
    url: string;
    size: number;
}
...
/**
 * A node that is used when a user is {@link https://talkjs.com/docs/Features/Message_Features/Mentions/ | mentioned}, for example `@alice`.
 *
...
/**
 * The state of a subscription when it is actively listening for changes
 *
 * @public
 */
 ...
 *
 * @remarks
 * Used in all Realtime API operations affecting that user, such as fetching or updating user data, or adding a user to a conversation.
...
export declare interface MessageRef {
     ...
     * Edits this message.
     *
     * @returns A promise that resolves when the operation completes. The promise will reject if the request is invalid, the message doesn't exist, you're not a participant, or your role does not give you permission to edit this message.
     */
    edit(params: string | EditTextMessageParams): Promise<void>;
    /**
 ...
 *
 * @remarks
 * The subscription is 'windowed'. By default, you subscribe to the 10 most recent messages.
...
export declare interface MessageSubscription {
     ...
     *
     * @remarks
     * This is either because we manually unsubscribed or because the subscription
...
/**
 * A node that is used when some text is meant to be hidden and not shown, due to {@link https://talkjs.com/docs/Reference/Concepts/Roles/#mask-unwanted-content | content suppression} settings.
 *
 * @public
 */
export declare interface SuppressedNode {
    type: "suppressed";
    text: string;
}
...
/**
 * Inside a `TextBlock`, there is a list of `TextNode` children.
 * A text node can either be a plain string, or one of a long list of other node types, which add extra formatting and custom behavior to the text.
 *
 * If the text nodes is not a plain string, it has a `type` field that indicates what kind of node it is.
 *
 * Some text nodes have their own list of `TextNode` children.
 * For example, a `MarkupNode` that renders all its children in bold.
 * This means that the text nodes form a tree structure.
 *
 * @public
 */
export declare type TextNode = string | SuppressedNode | EmojiNode | MarkupNode | CustomEmojiNode | BlockquoteNode | CodeSpanNode | LinkNode | MentionNode | AutoLinkNode | ActionLinkNode | ActionButtonNode | BulletListNode | BulletPointNode;
